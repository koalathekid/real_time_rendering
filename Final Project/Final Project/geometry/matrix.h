...
public:
  ...
  */
  float m(const uint32_t row, const uint32_t col) const {
   ...
   */
  float& m(const uint32_t row, const uint32_t col) {
  ...
  /**
   * Transforms a vector (normal or direction) by the matrix. Assumes
   * the homogeneous component is 0 so no translation occurs.
  ...
  Ray3 operator *(const Ray3& ray) const {
    return Ray3(*this * ray.o, *this * ray.d);
  }
